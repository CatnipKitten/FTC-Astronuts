...
public class Imu implements HardwareDevice, I2cController.I2cPortReadyCallback{
  ...
  /* Enable I2C Read Mode and address the bytes in the ReadCache using the following parameters: */
    ...
    */
    ...
    public Imu (HardwareMap currentHWmap, String configuredIMUname,
    ...
    */
        ...
        if (okSoFar) {
            ...
            snooze(500,'m');//Wait a decent interval until the IMU finishes its reset operations. IMU requires
        ...
        if (okSoFar) {
            ...
            okSoFar = okSoFar & autoCalibrationOK(10); //Check auto calibration with a timeout (seconds) on the wait
    ...
    private boolean i2cWriteImmediately(byte[]outboundBytes, int byteCount, int registerAddress) throws  RobotCoreException {
        ...
        snooze(250,'m');//Give the data time to go from the Interface Module to the IMU hardware
        return true;
    }

    private boolean autoCalibrationOK(int timeOutSeconds) throws RobotCoreException {
        ...
        while ((System.nanoTime() - calibrationStart) <= 60000000000L) {//Set a 1-minute overall timeout
            ...
            if (!readingEnabled) {//Start a stream of reads of the calibration status byte
                ...
                snooze(500,'m');//Give the data time to come into the Interface Module from the IMU hardware
                try {
